"""
initialize_with_perword_rt.py
-----------------------------
Same as initialize.py but keeps per-word RTs (RT_W1, RT_W2, RT_W3)
in the final item-level dataset instead of discarding W2/W3.

Outputs:
    data/word.csv       — full word-level dataset (same as original)
    data/item.csv       — item-level dataset with SUM_3RT + per-word RTs
"""
...
# extract per-word RTs before collapsing 
perword_rt = dataset.pivot_table(
    index=['SUB', 'ITEM', 'condition'],
    columns='critical_offset',
    values='RT',
    aggfunc='first'
).reset_index()
perword_rt.columns = ['SUB', 'ITEM', 'condition', 'RT_W1', 'RT_W2', 'RT_W3']

# SUM_3RT and trimming (same as original)
...
# merge per-word RTs back onto the collapsed dataset
dataset = pd.merge(dataset, perword_rt, on=['SUB', 'ITEM', 'condition'], how='left')

dataset = pd.merge(dataset, gpt_items, on=['ITEM', 'condition'])
dataset = dataset.sort_values(['SUB', 'ITEM'])
dataset.to_csv(os.path.join('data', 'item.csv'), index=False)

print(f"Done. Saved data/item.csv ({len(dataset)} rows)")
print(f"  RT_W1 non-null: {dataset['RT_W1'].notna().sum()}")
print(f"  RT_W2 non-null: {dataset['RT_W2'].notna().sum()}")
print(f"  RT_W3 non-null: {dataset['RT_W3'].notna().sum()}")
