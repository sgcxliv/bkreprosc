...
items_path = os.path.join('givenitems.csv')
items = pd.read_csv(items_path)

def prepare_data(items):
    words = items['sentence'].str.split().values.tolist()
    # Get critical word positions (convert to 0-indexed)
    critical_word_positions = (items['position'].values - 1).tolist()
    critical_words = items['critical_word'].tolist()
    keys = items['Index'].values.tolist()
