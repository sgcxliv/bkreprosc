...
run_dataset <- function(df, label) {
  ...
  for (i in seq_len(nrow(models))) {
    ...
    fits_agree[[m$short]] <- tryCatch(
      gam(make_formula(m$surp_col), data = df_agree, method = "REML"),
      error = function(e) {
        cat("ERROR fitting", m$pretty, "agree:", conditionMessage(e), "\n")
        NULL
      }
    )
    fits_disagree[[m$short]] <- tryCatch(
      gam(make_formula(m$surp_col), data = df_disagree, method = "REML"),
      error = function(e) {
        cat("ERROR fitting", m$pretty, "disagree:", conditionMessage(e), "\n")
        NULL
      }
    )
  }
...
# ------------------------------------------------------------------ #
# 6. Extract smooth curves from fitted GAMs
#
#    Uses plot.gam() internals rather than predict() so that CI width
#    correctly varies with data density (seWithMean=TRUE).
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
# 7. Extract and save curves for both datasets
# ------------------------------------------------------------------ #
...
cat("\nDone. Output files:\n")
cat("  bko21_gam_agree_vs_disagree_surprisal.pdf\n")
cat("  bkr21_gam_agree_vs_disagree_surprisal.pdf\n")
cat("  bko21_gam_summaries_agree_vs_disagree.txt\n")
cat("  bkr21_gam_summaries_agree_vs_disagree.txt\n")
cat("  bko21_gam_curves.csv\n")
cat("  bkr21_gam_curves.csv\n")
cat("\nColumns in curve CSVs: x, fit, se, se_upper, se_lower, model, condition, dataset, surp_col\n")
