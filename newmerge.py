...
surprisal = os.path.join(desktop_path, 'new_surprisals.csv')
items = os.path.join(desktop_path, 'ibex_items.csv')
...
merged_data = items.merge(
    surprisal,
    on=['critical_word', 'condition', 'position', 'ITEM'],  # Match on 4 Columns
    how='left'  # Keep all rows from items
)

# Construct output path for merged CSV
output_path = os.path.join(desktop_path, 'new_items_with_surprisal.csv')
